...
class ProjectDocumentController extends Controller
{
    ...
    private function getCompletionActIpIpHtml($project, $partner, $includeSignature, $includeStamp)
    {
        ...
        <head>
            <meta charset="UTF-8">
            <title>Акт завершения ремонта</title>
            <style>
                h1 {
                    font-size: 16pt;
                    text-align: center;
        ...
        <body>
            <h1>АКТ ЗАВЕРШЕНИЯ РЕМОНТА</h1>
            <div class="date">г. ' . ($project->city ?: 'Москва') . ', ' . $now->format('d.m.Y') . ' г.</div>
            
            <div class="content">
                <p>Индивидуальный предприниматель ' . $project->client_name . ', именуемый в дальнейшем «Заказчик», с одной стороны и Индивидуальный предприниматель ' . $partner->name . ', именуемый в дальнейшем «Исполнитель», с другой стороны, составили настоящий акт о нижеследующем:</p>
                
                <p>1. Исполнитель выполнил ремонтные работы по договору № ' . ($project->contract_number ?: 'б/н') . ' от ' . $now->format('d.m.Y') . ' г.</p>
                <p>2. Заказчик принял указанные работы.</p>
                <p>3. Стоимость выполненных работ составила ' . number_format($project->total_amount, 2, ',', ' ') . ' руб.</p>
                <p>4. Стороны взаимных претензий не имеют.</p>
            </div>
            
            <div class="signatures">
                <div class="party">
                    <div class="party-title">Заказчик:</div>
                    <p>ИП ' . $project->client_name . '</p>
                    <p>ИНН: ' . ($project->client_inn ?: '_________') . '</p>
                    <p>ОГРНИП: ' . ($project->client_ogrnip ?: '_________') . '</p>
                    <div class="signature">
                        <p>___________________ / ' . $project->client_name . ' /</p>
                    </div>
                </div>
                ...
                <div class="party">
                    ...
                    <p>ИП ' . $partner->name . '</p>
                    <p>ИНН: ' . ($partner->inn ?: '_________') . '</p>
                    <p>ОГРНИП: ' . ($partner->ogrnip ?: '_________') . '</p>
                    <div class="signature">
                        <p>___________________ / ' . $partner->name . ' /</p>
    ...
    private function getCompletionActFlIpHtml($project, $partner, $includeSignature, $includeStamp)
    {
        ...
        <body>
            ...
            <div class="content">
                <p>Гражданин РФ ' . ($project->client_name ?: 'ФИО НЕ УКАЗАНЫ') . ', именуемый в дальнейшем «Заказчик», с одной стороны и Индивидуальный предприниматель ' . $partner->name . ', именуемый в дальнейшем «Исполнитель», с другой стороны, составили настоящий акт о нижеследующем:</p>
                
                <p>1. Исполнитель выполнил ремонтные работы по договору № ' . ($project->contract_number ?: 'б/н') . ' от ' . ($project->contract_date ? $project->contract_date->format('d.m.Y') : $now->format('d.m.Y')) . ' г.</p>
                <p>2. Заказчик принял указанные работы.</p>
                <p>3. Стоимость выполненных работ составила ' . number_format($project->total_amount, 2, ',', ' ') . ' руб.</p>
                <p>4. Стороны взаимных претензий не имеют.</p>
            </div>
            ...
            <div class="signatures">
                <div class="party">
                    ...
                    <p>Паспорт: ' . $project->getFullPassportDataAttribute() . '</p>
                    <p>Адрес: ' . ($project->getFullRegistrationAddressAttribute() ?: 'не указан') . '</p>
                    <div class="signature">
                        <p>___________________ / ' . ($project->client_name ?: 'ФИО НЕ УКАЗАНЫ') . ' /</p>
                    </div>
                </div>
    ...
    private function getActIpIpHtml($project, $partner, $includeSignature, $includeStamp) {
        ...
        <body>
            ...
            <div class="content">
                ...
                <p>1. Исполнитель выполнил работы по договору № ' . ($project->contract_number ?: 'б/н') . ' от ' . ($project->contract_date ? $project->contract_date->format('d.m.Y') : $now->format('d.m.Y')) . ' г.</p>
                <p>2. Заказчик принял указанные работы.</p>
                <p>3. Стоимость выполненных работ составила ' . number_format($project->total_amount, 2, ',', ' ') . ' руб.</p>
                <p>4. Стороны взаимных претензий не имеют.</p>
            </div>
    ...
    private function getActFlIpHtml($project, $partner, $includeSignature, $includeStamp) {
        ...
        <body>
            ...
            <div class="content">
                ...
                <h3 style="text-align: center; margin-top: 0;">к Договору № ' . ($project->contract_number ?: 'б/н') . ' от ' . ($project->contract_date ? $project->contract_date->format('d.m.Y') : $now->format('d.m.Y')) . '</h3>
                
                <p>1. Все взаимные обязательства по ремонтно-строительным работам в рамках Договора № ' . ($project->contract_number ?: 'б/н') . ' от ' . ($project->contract_date ? $project->contract_date->format('d.m.Y') : $now->format('d.m.Y')) . ' выполнены полностью.</p>
                <p>2. Стоимость выполненных работ составляет ' . number_format($project->total_amount, 0, '', ' ') . ' (' . $this->num2str($project->total_amount) . ') рублей 00 копеек.</p>
                <p>3. Стороны претензий по выполнению условий Договора друг к другу не имеют.</p>
            </div>
    ...
    {
        // Формируем DOCX-совместимый HTML
        $docxHtml = '
        ...
        // Возвращаем документ как ответ с MIME-типом для MS Word
